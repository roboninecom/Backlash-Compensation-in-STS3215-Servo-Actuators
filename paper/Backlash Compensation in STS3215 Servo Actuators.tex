...
\author{Boris Kotov\\
Robonine\\
\texttt{hello@robonine.com}\\
\url{https://robonine.com}}
\date{} % arXiv обычно без даты
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.48\linewidth]{media/fig3_teststand_1.jpg}
    \hfill
    \includegraphics[width=0.48\linewidth]{media/fig3_teststand_2.jpg}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{media/fig5_sequence_stage1.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{media/fig6_sequence_stage2.png}
...
During each test, the motion sequence described above is executed while motor telemetry is sampled at approximately \SI{10}{\hertz}. For each cycle, two reference conditions are extracted:
\begin{itemize}
    \item \textbf{Loaded reference:} the encoder position while the pull motors apply the nominal \SI{0.3}{\kilo\gram force} to the lever in one direction.
    \item \textbf{Unloaded reference:} the encoder position immediately after both pull motors have relaxed and no external force is applied to the main lever.
\end{itemize}

For each pull direction (CW and CCW), two reference points are extracted:
\begin{itemize}
    \item \textbf{Unloaded (relaxed) state:} $p_{\mathrm{cw,unloaded}}$ and $p_{\mathrm{ccw,unloaded}}$,
    \item \textbf{Loaded (stretched) state:} $p_{\mathrm{cw,loaded}}$ and $p_{\mathrm{ccw,loaded}}$.
\end{itemize}
...
\end{align}
These two values represent the measured backlash in the absence of external force and under the applied counter-torque, respectively. Repeating the motion sequence multiple times yields sufficient samples to compute average backlash and its dispersion for each configuration (single motor, dual motor, dual motor with preload).
...
\begin{itemize}
    \item GitHub repository: \url{https://github.com/roboninecom/Backlash-Compensation-in-STS3215-Servo-Actuators}
\end{itemize}

\section*{Author Information}

Boris Kotov is a senior software engineer specializing in robotics control systems and embedded programming for autonomous platforms. Robonine is a company focused on developing accessible robotic solutions for education and research (\url{https://robonine.com}).

\begin{thebibliography}{9}

\bibitem{feetech_manual}
Feetech Automatic Technology Co., Ltd., ``STS3215 Serial Bus Servo User Manual,'' 2023. Available at \url{https://wiki.feetech.com/en/wiki/STS3215} (accessed: 20 Nov. 2025).

\bibitem{tao_backlash}
G.~Tao and P.~V.~Kokotovic, ``Adaptive Control of Systems with Backlash,'' \emph{Automatica}, vol.~29, no.~2, pp.~323--335, 1993, doi:~10.1016/0005-1098(93)90126-E.

\bibitem{nordin_survey}
M.~Nordin and P.~O.~Gutman, ``Controlling Mechanical Systems with Backlash---A Survey,'' \emph{Automatica}, vol.~38, no.~10, pp.~1633--1649, 2002, doi:~10.1016/S0005-1098(02)00047-X.

\bibitem{chandrasekar_measurement}
S.~Chandrasekar and K.~Srinivasan, ``Inferential Based Measurement of Backlash in Servo System,'' \emph{International Journal of Mechanical Sciences}, vol.~175, p.~105505, 2020, doi:~10.1016/j.ijmecsci.2020.105505.

\bibitem{hou_dual_motor}
L.~Hou, Y.~Cao, X.~Li, M.~Gao, and S.~Wang, ``Backlash Elimination Control for Robotic Joints with Dual-Motor Drive,'' \emph{Actuators}, vol.~13, no.~8, p.~291, 2024, doi:~10.3390/act13080291.

\bibitem{tan_precision}
J.~Tan, N.~Zhang, Y.~Zhao, and Z.~Wang, ``Nonlinear Robust Adaptive Precision Motion Control of Motor Servo Systems with Unknown Actuator Backlash Compensation,'' \emph{ISA Transactions}, vol.~138, pp.~237--251, 2023, doi:~10.1016/j.isatra.2023.02.022.

\bibitem{zhou_deadzone}
L.~Zhou, Z.~Mao, and J.~Huang, ``Robust Stabilization of Nonlinear Uncertain Plants with Backlash or Dead Zone in the Actuator,'' \emph{IEEE Transactions on Control Systems Technology}, vol.~10, no.~2, pp.~158--166, 2002, doi:~10.1109/87.987060.
